...
function paginate(allEdges, args) {
  return {
...
function edgesToReturn(allEdges, args) {
  ...
  var edges = applyCursorsToEdges(allEdges, before, after);
...
function applyCursorsToEdges(allEdges, before, after) {
  ...
  if (after) {
    ...
    allEdges.some(function (edge, index) {
  ...
  if (before) {
    ...
    if (beforeEdge) {
      edges = allEdges.slice(0, _cursorIndex);
    }
  }
